 ...
 * - [themeId] 选择三套配色之一（莫兰迪/青瓷/墨韵），见 [AppTheme]；未知值回退默认莫兰迪。
...
/** 兼容旧调用（无主题参数，按默认莫兰迪渲染）；新代码应改用带 themeId 的重载。 */
@Composable
fun PersonalFaTheme(
    darkTheme: Boolean = isSystemInDarkTheme(),
    content: @Composable () -> Unit,
) = PersonalFaTheme(themeId = AppTheme.MORANDI.id, darkTheme = darkTheme, content = content)
