...
- 当前分支：`main`。**本地领先 origin/main 7 个提交，均未推送**（用户自行 push）：迁移合并 `0d69d8b` + 文档 `c9adc2e` + 本批三项 feat + 本批 docs 提交。
- 本轮四个批次均已提交（混合工作区按功能拆 3 个 feat 提交，真机验证的是最终合并树）：
...
- 验证：完整构建成功，**541 tests / 0 failures / 0 errors**（基线 492 → 净增 49）。已验收 APK：`apk/RICHWIN-AI多配置-20260910.apk`（覆盖安装迁移/编辑生效/添加自定义+切生效/key 隔离/备份恢复五场景真机通过）。SHA-256：`76aefa71353e1dd96dd500dd2c30a6c7493be8f370b7b5b9a8b95f88dbbdf8fc`。APK 被 gitignore 排除，仅本地保存。中间验收包：`RICHWIN-国际财经提醒-20260909.apk`、`RICHWIN-提醒删除-20260909.apk`。
- **数据库仍为 v13，本批零 schema 改动**（档案存 DataStore JSON，免 v14）。destructive fallback 保持移除；不要未经重新验证恢复数据库预打开逻辑（曾在用户手机闪退）。
...
- 下次先读本文件并检查 `git status` / 当前分支。下一项功能尚未指定。继续遵守分批交付、真机认可后提交、只 commit 不 push。
...
- d. 提醒：新股(东财申购日历)/持仓到期/持仓波动/市场指数/**国际财经快讯(RSS 主备双源+关键词打分)** + 提醒删除（滑动/多选/全部） ✅
...
2. ~~国际财经提醒~~ ✅ 已完成（2026-09-09，RSS 主备双源 + NewsScorer 关键词打分）。
3. P7 打磨：深色模式细化、无障碍、性能、E2E、发布签名；数据库当前仅支持 12→13，更早版本须先取得可靠历史证据再评估支持范围。
...
- **2026-09-10 四项改动分 3+1 个 commit 本地提交（主分支 `main`，未推送）**：
  - `c2f2a03` feat: 国际财经快讯提醒（含提醒删除 UI，混合批次按文件级归属合并提交）
  - `d78098e` feat: AI API Key 加密入库 — Keystore 文件方案改为数据库键值表存储
  - `9635053` feat: AI 助手多配置档案 — 每服务商一档+多自定义档案，Key 按档案独立加密存储
  - 541 个单元测试全绿；APK：`apk/RICHWIN-AI多配置-20260910.apk`（用户已真机五场景验证通过）
